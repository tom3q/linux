...
static const char *cable_names[] = {
	[EXTCON_CABLE_USB_HOST] = "USB-Host",
	[EXTCON_CABLE_USB] = "USB",
	[EXTCON_CABLE_UART] = "UART",
	[EXTCON_CABLE_CHARGER] = "Charger",
	[EXTCON_CABLE_JIG] = "JIG",
	[EXTCON_CABLE_AUDIO] = "Audio",
	[EXTCON_CABLE_VIDEO] = "Video",
	[EXTCON_CABLE_TTY] = "TTY-converter",
				  ...
				  u16 mask, bool attached)
{
	while (mask) {
		...
		while (cables) {
			...
			extcon_set_cable_state_(&usbsw->edev, cable, attached);
			cables &= ~BIT(cable);
		}
			 ...
			 const struct i2c_device_id *id)
{
	...
	/* External connector */
	info->edev.name = "fsa9480";
	info->edev.dev.parent = &client->dev;
	info->edev.supported_cable = cable_names;
	...
	ret = request_threaded_irq(client->irq, NULL, fsa9480_irq_handler,
...
{
	struct fsa9480_usbsw *info = i2c_get_clientdata(client);
